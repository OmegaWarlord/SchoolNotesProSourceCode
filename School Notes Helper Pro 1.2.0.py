...
defaultpath = ospath.expanduser("~")+'\\Documents\\.schoolnotespro'
customPath = defaultpath+'\\custom'
hwFile = ospath.expanduser("~")+'\\Documents\\.schoolnotespro\\hwFile.wlist'
configFile = ospath.expanduser("~")+'\\Documents\\.schoolnotespro\\settingsFile.wlist'
...
def AddHomework():
    dtsd = [dt.strftime('%D') for dt in
       ...
       timedelta(days=1))]
    global homeworklist, homeworkdue
    hwitem = e.multenterbox("Name & Class of Assignment:", title, ['Name:', 'Class:']) # todo Add calendar date picker
    def thing():
        global datedue
        datedue = e.multenterbox("Please center the duedate in Month/Day/Year format EX: 01/01/20", title, ['MM/DD/YY', 'HH:MM', 'AM/PM'])

    thing()

    while True:
        try:
            gettimes = datedue[0].split('/')
            if len(str(gettimes[0])) == 1: gettimes[0] = f'0{gettimes[0]}'
            if len(str(gettimes[1])) == 1: gettimes[1] = f'0{gettimes[1]}'
            datedue[0] = f'{gettimes[0]}/{gettimes[1]}/{gettimes[2]}'
        except IndexError:
...
def RemoveHomework():
    global homeworklist, displayHomework, counter3
...
def SaveHomework():
    global hwFile, homeworklist, counter3
...
def EditHomework():
    global displayHomework, homeworklist, counter3
    value=str((displayHomework.get('active')))
    Index = homeworklist.index(value)
    b = value.split(' ')
...
def clock():
    ...
    time = datetime.datetime.now().strftime("%I:%M:%S %p | %D")
    if time[0] == "0":
        time = time[1:]
    time = f"Time: {time}"
    ...
    if weekday == "Monday":
        ...
        allclasses = '\n'.join(str(elem) for elem in allclasses)
        alltimes = '\n'.join(str(elem) for elem in alltimes)
    elif weekday == "Saturday" or weekday == "Sunday": allclasses = 'No School'; alltimes = 'N/A'
    ...
    for j in homeworklist:
        ...
        homeworkdue.insert(index, TimeLeft)
        TimeLate = now - duedate
        if 3 > counter3:
            if now >= duedate:
                if '[LATE]' in str(homeworklist[index]): pass
                else: homeworklist[index] = (j + ' [LATE]')
            Z = [x for _,x in sorted(zip(homeworkdue,homeworklist))]
            for z2 in Z:
                index = Z.index(z2)
                displayHomework.delete(index)
                if '$_' in str(homeworklist[index]):
                    displayHomework.insert(index,str(z2).replace('$_', ' '))
                else: displayHomework.insert(index,z2)
    counter3 += 1
    homeworkdue=[]
    if counter3 == 50:
        counter3 = 0
