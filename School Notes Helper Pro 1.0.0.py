...
def AddHomework():
    ...
    if '' in hwitem:
        AddHomework()
    homeworklist.append(f'[{hwitem[2]}] {hwitem[0]} - Due {hwitem[1]}')
    displayHomework.insert('end', f'[{hwitem[2]}] {hwitem[0]} - Due {hwitem[1]}')
    SaveHomework()
def RemoveHomework():
    global homeworklist
...
lab = tk.Label(root, width='30', textvariable=displayClasses, anchor='w', justify="left", background="#FFFFFF", fg="black", font=("Arial", 10))
...
def clock():
    ...
    from datetime import datetime, timedelta
    dts12 = [dt.strftime('%I:%M') for dt in
       datetime_range(datetime(2016, 1, 1), datetime(2016, 1, 2),
       timedelta(minutes=1))]
